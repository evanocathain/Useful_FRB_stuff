...
for (iword=0; iword<6; iword++)
  ...
  for (ibit=0; ibit<16; ibit++) /* loop over input channels */
    ...
    if(nshift != 0) 
      {
      nshift = (int)fmod(((double)(nshift-1+8)),16.0);
      bit1 = 1;
      bit1 = bit1<<(nshift);  /* bit in output word */
      nshift = (int)fmod(((double)(ibit+8)),16.0);
      bit2 = 1;
...
for (iword=0; iword<6; iword++)
  ...
  for (ibit=0; ibit<16; ibit++)
    ...
    if(nshift != 0) 
      ...
      if(mtab[iword][j] !=0) printf("btab %d %d %4.4x\n",iword,j,mtab[iword][j]);
    } 
  }
...
for (iword=0; iword<6; iword++)
  ...
  for (ibit=0; ibit<16; ibit++)
    ...
    if(nshift != 0) 
      ...
      if(btab[iword][j] !=0) printf("btab %d %d %4.4x\n",iword,j,btab[iword][j]);
    } 
  }
